    ...
    <div className="pb-wrapper" style={{ width: '100%', padding: '2rem 2rem 5rem' }}>
      ...
      <button
        onClick={() => {
          ...
          const field = (label: string, key: string, data: Record<string, unknown>) => {
            const val = getStr(data, key)
          ...
          push('**Brand Playbook**')
          push('')
          push(`**Creator:** ${brandName}`)
          if (handle) push(`**Handle:** @${handle}`)
          push('')

          // ── Module 01: Brand Foundation ──
          push('---')
          push('## Module 01 — Brand Foundation')
          push('')

          push('### Brand Journey')
          field('Desired Outcome', 'bf_journey_outcome', bf)
          field('Why', 'bf_journey_why', bf)
          field('Known For', 'bf_journey_known', bf)
          field('What You Do', 'bf_journey_do', bf)
          field('What You\'re Learning', 'bf_journey_learn', bf)
          field('Brand Statement', 'bf_journey_statement', bf)
          push('')

          push('### Mission')
          field('What You Love', 'bf_ikigai_love', bf)
          field('What You\'re Good At', 'bf_ikigai_good', bf)
          field('What the World Needs', 'bf_ikigai_world', bf)
          field('What You Can Be Paid For', 'bf_ikigai_paid', bf)
          field('Core Mission Statement', 'bf_core_mission', bf)
          push('')

          push('### Avatar')
          field('Avatar Story', 'bf_av1_story', bf)
          field('Avatar Goals', 'bf_av1_goals', bf)
          field('Avatar Struggle', 'bf_av1_struggle', bf)
          field('Desired Outcome', 'bf_av1_desired', bf)
          field('Avatar Statement', 'bf_av1_statement', bf)
          push('')

          push('### Values')
          for (let i = 1; i <= 6; i++) {
            const name = getStr(bf, `bf_val${i}_name`)
            const practice = getStr(bf, `bf_val${i}_practice`)
            if (name) push(`**Value ${i}:** ${name}${practice ? ` — ${practice}` : ''}`)
          }
          push('')

          push('### Content Pillars')
          for (let i = 1; i <= 5; i++) {
            const name = getStr(bf, `bf_pillar${i}_name`)
            const sub = getStr(bf, `bf_pillar${i}_sub`)
            if (name) push(`**Pillar ${i}:** ${name}${sub ? ` — ${sub}` : ''}`)
          }
          push('')

          push('### Origin Story')
          field('Part 1', 'bf_story1', bf)
          field('Part 2', 'bf_story2', bf)
          field('Part 3', 'bf_story3', bf)
          field('Part 4', 'bf_story4', bf)
          field('Origin Story', 'bf_origin_story', bf)
          push('')

          push('### Brand Vision')
          field('3-Year Vision', 'bf_vision_3yr', bf)
          field('Day in the Life', 'bf_vision_day', bf)
          field('Impact', 'bf_vision_impact', bf)
          field('Legacy', 'bf_vision_legacy', bf)
          field('Brand Vision Statement', 'bf_brand_vision', bf)
          push('')

          // ── Module 02: Visual World ──
          push('---')
          push('## Module 02 — Your Visual World')
          push('')

          push('### Creator Analysis')
          field('Creators You Study', 'vw_ca_creators', vw)
          field('Patterns You Notice', 'vw_ca_patterns', vw)
          field('What You\'d Do Differently', 'vw_ca_different', vw)
          field('What\'s Your Own', 'vw_ca_own', vw)
          field('Gap in the Market', 'vw_ca_gap', vw)
          push('')

          push('### Mood Board')
          field('Colors', 'vw_mb_colors', vw)
          field('Lighting', 'vw_mb_lighting', vw)
          field('Mood', 'vw_mb_mood', vw)
          field('Textures', 'vw_mb_textures', vw)
          field('Movie Reference', 'vw_mb_movie', vw)
          field('Time of Day', 'vw_mb_time', vw)
          field('Place', 'vw_mb_place', vw)
          push('')

          push('### Color Palette')
          field('Primary Color', 'vw_color_primary', vw)
          field('Secondary Color', 'vw_color_secondary', vw)
          field('Accent Color', 'vw_color_accent', vw)
          field('Neutral', 'vw_color_neutral', vw)
          field('Palette Name', 'vw_color_name', vw)
          push('')

          push('### Typography')
          field('Primary Font', 'vw_typo_primary', vw)
          field('Body Font', 'vw_typo_body', vw)
          push('')

          push('### Your Perspective (Shot System)')
          field('Locations', 'vw_shot_e1_location', vw)
          field('Setting Vibe', 'vw_shot_e1_vibe', vw)
          field('Setting Statement', 'vw_shot_e1_statement', vw)
          field('Mood', 'vw_shot_e2_mood', vw)
          field('Lighting', 'vw_shot_e2_lighting', vw)
          field('Time of Day', 'vw_shot_e2_time', vw)
          field('Mood Statement', 'vw_shot_e2_statement', vw)
          field('Color Grade', 'vw_shot_e3_grade', vw)
          field('Color Reference', 'vw_shot_e3_ref', vw)
          field('Props & Objects', 'vw_shot_e4_objects', vw)
          field('Textures', 'vw_shot_e4_textures', vw)
          field('Wardrobe', 'vw_shot_e4_wardrobe', vw)
          field('Never Include', 'vw_shot_e4_never', vw)
          push('')

          // ── Module 03: Content ──
          push('---')
          push('## Module 03 — Create Your Content')
          push('')

          push('### Strategy')
          field('Content Goal', 'ct_strategy_goal', ct)
          field('Next Step for Audience', 'ct_strategy_next_step', ct)
          field('Pain / Problem You Solve', 'ct_strategy_pain_problem', ct)
          field('Unique Solution', 'ct_strategy_unique_sol', ct)
          field('Credibility', 'ct_strategy_credibility', ct)
          push('')

          push('### Sustainability')
          field('Hours Per Week', 'ct_sustain_week_hours', ct)
          field('What Energizes You', 'ct_sustain_energize', ct)
          field('What Drains You', 'ct_sustain_drain', ct)
          field('Content Cadence', 'ct_sustain_cadence', ct)
          field('Primary Platform', 'ct_sustain_primary', ct)
          field('Secondary Platform', 'ct_sustain_secondary', ct)
          push('')

          push('### Batching System')
          field('Film Day', 'ct_batch_film_day', ct)
          field('Pieces Per Session', 'ct_batch_count', ct)
          field('Setup', 'ct_batch_setup', ct)
          push('')

          push('### Idea Generation')
          for (let p = 1; p <= 5; p++) {
            const pillar = getStr(ct, `ct_ig_pillar${p}`)
            if (!pillar) continue
            push(`**Pillar ${p}: ${pillar}**`)
            for (let i = 1; i <= 4; i++) {
              const idea = getStr(ct, `ct_ig_p${p}i${i}`)
              if (idea) push(`  - ${idea}`)
            }
          }
          push('')
          ...
          push('### Storytelling')
          field('Story Idea', 'ct_story_idea', ct)
          field('Hook', 'ct_story_hook', ct)
          field('Problem', 'ct_story_prob', ct)
          field('Journey', 'ct_story_journey', ct)
          field('Lesson', 'ct_story_lesson', ct)
          field('CTA', 'ct_story_cta', ct)
          push('')

          // ── Module 04: Launch ──
          push('---')
          push('## Module 04 — Launch')
          push('')

          push('### Bio')
          field('Username', 'la_bio_username', la)
          field('IG Name', 'la_bio_ig_name', la)
          field('Bio Line 1', 'la_bio_line1', la)
          field('Bio Line 2', 'la_bio_line2', la)
          field('Bio Line 3', 'la_bio_line3', la)
          field('Bio Line 4', 'la_bio_line4', la)
          field('Link', 'la_bio_link', la)
          push('')

          push('### Funnel')
          field('Platforms', 'la_funnel_platforms', la)
          field('Lead Magnet', 'la_funnel_lead_magnet', la)
          field('Email Platform', 'la_funnel_email_platform', la)
          field('Newsletter Frequency', 'la_funnel_newsletter_freq', la)
          field('CTA', 'la_funnel_cta', la)
          field('Offer', 'la_funnel_offer', la)
          field('Price', 'la_funnel_price', la)
          field('Conversion Path', 'la_funnel_conversion', la)
          push('')

          push('### Lead Magnet')
          field('Name', 'la_lm_name', la)
          field('Topic', 'la_lm_topic', la)
          field('Bridge to Offer', 'la_lm_offer_bridge', la)
          field('Format', 'la_lm_format', la)
          field('Big Win', 'la_lm_big_win', la)
          field('Outline', 'la_lm_outline', la)
          field('CTA', 'la_lm_cta', la)
          field('Tool', 'la_lm_tool', la)
          field('Delivery', 'la_lm_delivery', la)
          push('')

          push('### Launch Content')
          push('**Post 1 — Your Story**')
          field('Why', 'la_lc_story_why', la)
          field('Challenge', 'la_lc_story_challenge', la)
          field('Turning Point', 'la_lc_story_turning', la)
          field('Lesson Learned', 'la_lc_story_learned', la)
          field('Hook', 'la_lc_story_hook', la)
          field('CTA', 'la_lc_story_cta', la)
          push('')
          push('**Post 2 — Positioning Deep Dive**')
          field('Core Belief', 'la_lc_pos_belief', la)
          field('Bold Claim', 'la_lc_pos_claim', la)
          field('Old Belief', 'la_lc_pos_oldbelief', la)
          field('New Belief', 'la_lc_pos_newbelief', la)
          field('Anchor', 'la_lc_pos_anchor', la)
          push('')
          push('**Post 3 — Authority Masterclass**')
          field('Subject', 'la_lc_mc_subject', la)
          field('Audience', 'la_lc_mc_audience', la)
          field('Authority Credential', 'la_lc_mc_authority', la)
          field('Hook', 'la_lc_mc_hook', la)
          push('')

          push('### Goals (90 Days)')
          field('Content Frequency', 'la_goal_content_freq', la)
          field('Platforms', 'la_goal_content_platforms', la)
          field('Content Goal', 'la_goal_content', la)
          field('Follower Goal', 'la_goal_followers', la)
          field('Email List Goal', 'la_goal_email', la)
          field('Audience Goal', 'la_goal_audience', la)
          field('Offer Goal', 'la_goal_offer', la)
          field('Sales Goal', 'la_goal_sales', la)
          field('Revenue Goal', 'la_goal_revenue', la)
          field('System Priority', 'la_goal_system_priority', la)
          field('System', 'la_goal_system', la)
          field('Review Date', 'la_goal_review_date', la)
          field('Accountability', 'la_goal_accountability', la)
          push('')

          push('---')
          push('*Generated by FSCreative™ Brand Playbook — fscreative.live*')
