...
describe('POST /api/waterfall', () => {
  ...
  it('returns 401 JSON when no auth session', async () => {
    ...
    const { POST } = await import('@/app/api/waterfall/route')
    const request = new NextRequest('http://localhost:3000/api/waterfall', {
      method: 'POST',
      body: JSON.stringify({ transcript: 'some transcript text' }),
  ...
  it('returns 413 JSON when body exceeds 10,000 chars', async () => {
    const { POST } = await import('@/app/api/waterfall/route')
    const request = new NextRequest('http://localhost:3000/api/waterfall', {
      method: 'POST',
      body: JSON.stringify({ transcript: 'x'.repeat(10_001) }),
      headers: { 'Content-Type': 'application/json' },
    })
  ...
  it('forwards request to waterfall-analyzer edge function and returns response', async () => {
    const edgeResponse = {
      waterfall: { hooks: [], content: [] },
    ...
    expect(url).toMatch(/\/functions\/v1\/waterfall-analyzer$/)
